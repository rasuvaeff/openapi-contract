...
- `ext-bcmath` опционально: с ним бэкенд вычисляет `multipleOf` в десятичной
  арифметике, без него — в плавающей точке с допуском `1e-14`, что точно для
  любого делителя, который встретится в документе, и может расходиться на
  краях этой точности
