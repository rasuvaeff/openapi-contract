...
- никаких расширений сверх `json`: `multipleOf` судится по десятичной
  записи, которой документ и сообщение записали числа (`64.1` — кратное
  `0.1`; `64.10000000000001` — нет), точно и независимо от того, загружен ли
  `ext-bcmath` — собственная арифметика бэкенда, читавшая распарсенные
  double и отвечавшая по-разному с расширением и без, не используется (#151)
...
multipart-части, — компилируется при сборке контракта, в том направлении, в
котором будет прочитан, поэтому схема, которую пакет не может вычислить
(assertion-ключевое слово вне матрицы поддержки вроде `patternProperties`,
`$schema` с другим диалектом, `exclusiveMinimum` числом под OAS 3.0,
`pattern`, который бэкенд не разбирает, — на любой глубине), это
`InvalidContract` из фабрики и никогда — из `validate*()`.
...
`UnsupportedSerialization`), `InvalidLimits`, `UnknownOperation` и
`ContractViolation`. Базовые классы остались прежними —
`\InvalidArgumentException` и `\RuntimeException`, поэтому существующие catch
продолжают работать.
...
`InvalidLimits` — `\InvalidArgumentException`, реализующим `ContractException`.
...
документа, и шейпы, которые конструктор принимает, это выход компилятора, а
не проверяемый вход — ни один публичный путь не валидирует собранную руками
операцию. Конструктор при этом — публичный API, потому что потребители
собирают операции руками в своих тестах: он append-only (минорный релиз может
добавить параметр с дефолтом в конец, но не переставить и не убрать), поэтому
вызывать его нужно именованными аргументами. Шейпы, которые импортирует
потребитель — `CompiledParameter`, а для `$requestBody`/`$responses`
`CompiledRequestBody`/`CompiledResponses`, — для него доступны на чтение, и
минорный релиз может добавить в них ключи. `CompiledRequestBody` и
`CompiledResponses` — это Request Body Object и Responses Object в том виде,
в каком их оставляет компилятор: каждый `$ref` на пути к схеме разрешён,
`required` — булево, `content` ключуется media type, `encoding` и `headers` —
именем свойства и заголовка, а каждая `schema` — булево или карта ключевых
слов (пустая карта — неограниченная схема); что документ написал рядом с
этими ключами, сохраняется как есть. `CompiledResponses` ключуется кодом
статуса так, как его читает PHP (`"200"` это `int 200`), диапазоном `NXX`
или `default`. Скомпилированные параметры несут
...
шаблонным и режет путь по сырому `/` до того, как декодирует каждый сегмент
ровно один раз, — поэтому percent-закодированный разделитель остаётся частью
своего сегмента и никогда не становится границей: `/pets/a%2Fb` совпадает с
`/pets/{name}`, где `name` декодируется в `a/b` — значение, которое получит
приложение, — а `/a%2Fb/x` не совпадает с `/a/b/x`. Завершающий слэш — часть пути: `/pets` и `/pets/` это разные ресурсы, как их
...
его пары режутся по `;` (и необязательному пробелу, который RFC 6265 после
него допускает) и никогда по `&` — это обычный cookie-octet: `sid=abc&def` —
одна cookie со значением из семи символов. Строка с провода читается как
`integer` или `number` только когда пишет число по грамматике JSON-числа
(`-?(0|[1-9][0-9]*)(.[0-9]+)?([eE][+-]?[0-9]+)?`, ничего до и после): `.5`,
`5.`, ` 5`, `5\n` и `0x1A` остаются строками и не проходят схему, а целое за
пределами диапазона PHP сохраняет величину как float, а не насыщается.
...
компиляции. Две вещи проверяются только по форме: `oauth2`-схема, чей объект
`flows` не объявляет ни одного потока, компилируется в пустой `flows`, а
URL-поля (`tokenUrl`, `authorizationUrl`, `refreshUrl`, `openIdConnectUrl`)
обязаны быть непустыми строками, но как URL не разбираются.
...
только присутствие. `readOnly`/`writeOnly` применяются направленно — что это
значит, см. [Проверка одной схемы](#проверка-одной-схемы). Root `security`
...
(`type: string`, с `minLength`/`maxLength`/`pattern` и любым проверяемым `format`) сырое тело
...
неверно.
JSON-тело декодируется с бюджетом вложенности в 64 уровня; тело глубже
...
`Contract::accepts()` отвечает для одного Schema Object этого документа на
вопрос, который задаёт валидация: удовлетворяет ли значение схеме? Это та же
скомпилированная схема и тот же бэкенд, которыми пользуются
`validateRequest()` и `validateResponse()`, и кэш компиляции контракта
переиспользуется — проверка многих значений по одной схеме компилирует её
один раз.
...
$contract->accepts(42, $schema);                             // направление request
$contract->accepts($value, $schema, SchemaDirection::Response);
```

Направление — не украшение. Свойство `readOnly` не обязательно в запросе, а
`writeOnly` — в ответе: до того, как значение судят, свойство теряет свою
запись в `required` для чужого направления и сохраняет подсхему — оно
остаётся объявленным и типизированным, поэтому запрос, несущий `readOnly`
`id`, судится по схеме `id`, а закрытый объект (`additionalProperties: false`)
по-прежнему его принимает, ровно как в обеих спецификациях («the required
will take effect on the response only»). Перезапись рекурсивно проходит
`properties`, `items`, `additionalProperties` и композиционные ключевые слова,
не трогает `not`, и именно из-за неё одно и то же значение и одна и та же
схема отвечают по-разному в двух направлениях.

Сама перезапись экспортирована, чтобы потребитель, который строит значения
для одного направления, строил их по той схеме, по которой их проверят, а не
по копии правила:
...
`effective()` возвращает ровно то, что компилируют валидаторы — неподвижную
точку самой себя, независимую от диалекта, с каждым членом, который она не
читает, переданным как записан.

Значение судится так, как бэкенд читает JSON: объект — это `stdClass`, каким
его отдаёт `json_decode()` без `associative: true`. Ассоциативный массив PHP —
это JSON-*массив*, который не примет ни одна схема `type: object`. Параметр
едет по проводу строкой и декодируется до того, как его судят, — передавать
надо декодированное значение, а не проводное написание.

Потребитель, у которого есть `Operation` и нет контракта, берёт `SchemaCheck`
и называет диалект, который несёт операция:
...
`Operation::$dialect` заполняет компиляция из версии `openapi` документа —
`SchemaDialect::OpenApi30` или `SchemaDialect::OpenApi31`, — потому что
диалект решает, как читается схема: 3.0 пишет nullability как
`nullable: true`, а исключающие границы булевыми, 3.1 — типом-объединением и
числами. Схема, которую диалект прочитать не может, поднимает
`InvalidContract`, а не читается молча как что-то другое.

Две вещи, с которыми может столкнуться схема, написанная руками, и не может
схема из документа — компилятор решает их при загрузке. `$ref` резолвится
только внутри самой схемы (`#/$defs/…`); ссылка на `#/components/…`, в другой
файл или на URL поднимает `InvalidContract`. Схема, которую нельзя закодировать
в JSON — `NAN`/`INF`, битый UTF-8, больше 512 уровней вложенности, — тоже
поднимает `InvalidContract`, как и список там, где ожидался объект. Кеш
компиляций за обоими методами ключуется схемой и не вытесняется: `Contract`
держит конечное число схем, а `SchemaCheck`, которому скармливают бесконечный
поток различных схем, растёт вместе с ним — один экземпляр на документ, а не
на генератор.

`SchemaCheck::isMultipleOf($value, $divisor)` — сам вердикт `multipleOf`,
статический и экспортированный для потребителя, которому его нужно
предсказать: генератор, решающий, допускает ли `number`-ветвь `oneOf` целое,
которое он собирается оставить в `integer`-ветви, спрашивает здесь, а не
держит вторую копию правила. Судит по десятичной записи чисел (`64.1` —
кратное `0.1`, `64.10000000000001` — нет), точно, на любой машине.
...
Три расхождения со спекой сделаны осознанно и зафиксированы:

| Где | Спека | Пакет |
|---|---|---|
| Percent-encoded разделитель внутри значения `pipeDelimited`/`spaceDelimited` | `\|` и пробел внутри значения MUST быть percent-encoded, поэтому `?ids=a%7Cb` — один элемент `a\|b` | складывает закодированную форму в разделитель и читает два элемента — ровно так же читает PHP-приложение, разбирающее тот же query, а согласие с приложением и есть смысл валидатора. Значение с разделителем внутри выразить нельзя |
| Header Object с `name` или `in` | оба MUST NOT be specified | игнорируются, а не отвергаются: имя заголовка в любом случае берётся из ключа карты |
| `example` и `examples` на одном объекте | взаимоисключающие | оба сохраняются как аннотации и отдаются потребителю; валидация не читает ни одно |
...
`format` проверяется там, где у бэкенда есть проверка, и остаётся аннотацией
везде, кроме этого — значение с неизвестным или непроверяемым форматом за
него никогда не отвергается:

| Тип | Проверяется | Только аннотация |
|---|---|---|
| `string` | `date`, `time`, `date-time`, `duration`, `uri`, `uri-reference`, `uri-template`, `regex`, `ipv4`, `ipv6`, `uuid`, `email`, `hostname`, `idn-hostname`, `idn-email`, `iri`, `iri-reference`, `json-pointer`, `relative-json-pointer` | `byte`, `binary`, `password` и любое другое значение |
| `integer` | `int32` (−2³¹ … 2³¹−1), `int64` (−2⁶³ … 2⁶³−1; значение за пределами целого PHP приходит как float и судится по величине) | любое другое значение |
| `number` | — | `float`, `double` и любое другое значение |
