...
может развернуться в сотни миллионов узлов. Бюджет резолюции
(`resolvedNodes`) ограничивает работу инлайнинга: компонент, общий для многих
операций, посещается по разу на каждое использование, поэтому большое
описание стоит больше резолюции, чем в нём узлов — REST API GitHub на 1 239
операций требует около 360 000 из миллиона по умолчанию.
...
$contract = Contract::fromFile('openapi.yaml', new Limits(
    ...
    resolvedNodes: 4_000_000,          // по умолчанию 1 000 000
));
```
...
этими ключами, сохраняется как есть. Схему, ссылающуюся на саму себя —
дерево, чьи `children` тоже деревья, тред, вложенную ошибку, — инлайном не
записать, поэтому члены каждого цикла ссылок сохраняются как `$defs` схемы,
названные по их JSON Pointer (`#/components/schemas/Node` становится
`components.schemas.Node`, член другого файла — `a.json:Node`), а обратная
ссылка на них — локальный `{$ref: '#/$defs/…'}`, несущий `type` и `format`
цели; схема, с которой цикл начинается, инлайнится там, где встречена
впервые, и заодно сохраняется как def. `$ref`-ветвь `oneOf`/`anyOf` с
`discriminator` сохраняется тем же локальным `$ref` на любой глубине, чтобы
ветвь удержала имя компонента, с которым диагностика сверяет значение
дискриминатора; ветвь, написанная inline, остаётся inline. У схемы без
цикла, общего компонента и дискриминированного union `$defs` нет. Цикл
ссылок вне схемы — Path Item или Response, доходящий до самого себя, —
отвергается, как и цикл, в котором нет ни одной схемы.

Компонент, достигнутый повторно в любом месте документа — DAG из общих
компонентов, каким является крупная секция `components`, — тоже не
разрешается дважды: первое разрешение хранится на весь документ, а каждое
дальнейшее использование ниже уровня, который читают wire-декодеры
(значения `properties`, `items`, `additionalProperties` самой схемы и её
прямых свойств), становится тем же локальным `{$ref: '#/$defs/…'}`, причём
defs, собранные первым разрешением, едут с ним, чтобы ссылки разрешались.
Использования, с которых декодеры читают словари, остаются
проинлайненными: отложенный узел несёт только `type` и `format`.
Опубликованный Stripe `spec3.json` (8 МБ, OAS 3.0, несколько крупных схем,
достигаемых сотнями других) так грузится; позиций схемы в нём тысячи,
поэтому нужны поднятые лимиты — `documentBytes` за 8 МБ и `resolvedNodes`
в десятки миллионов — а компиляция стоит секунды и гигабайты, что
отслеживается отдельно.
 `CompiledResponses` ключуется кодом
...
`properties`, `items`, `additionalProperties`, композиционные ключевые слова и
`$defs`, не трогает `not`, и именно из-за неё одно и то же значение и одна и та же
