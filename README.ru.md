...
Направление — не украшение. Свойство `readOnly` не входит в запрос, а
`writeOnly` — в ответ; каждое отбрасывается вместе со своей записью в
`required` до того, как значение судят, поэтому одно и то же значение и одна
и та же схема отвечают по-разному в двух направлениях.
